...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_GRE3DRosette
global VCtl
global VVar
CV1=1e-3;
CV10=0;
...
p.rfFreq=0;
p.rfPhase=pi/2;
p.tEnd=VCtl.TE/2+0.4e-3;
p.tStart=VCtl.TE/2+0.1e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p=[];
%--------------------
p.Notes='Rosette encoding';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.dt=1e-4;
if strcmp(p.Switch,'on')
[GyAmp1,GyTime1]=GyRosette(p);
GyAmp=[GyAmp GyAmp1];
...
p.DupSpacing=0;
p.Duplicates=1;
p.Notes='Rosette readout';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.dt=1e-4;
if strcmp(p.Switch,'on')
[ADC1,ADCTime1]=ADCRosette(p);
ADC=[ADC ADC1];
